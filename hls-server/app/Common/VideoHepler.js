...
async function ConvertsVideo (file_name) {
    const filename = file_name.split('.')
    cv.setFfmpegPath(ffmpegInstaller.path);

    if (!fs.existsSync(Helpers.publicPath(`video/${filename[0]}`))){
        fs.mkdirSync(Helpers.publicPath(`video/${filename[0]}`));
    }
    ...
    cv(Helpers.tmpPath(`uploads/${file_name}`), { timeout: 432000 }).addOptions([
        ...
        '-hls_list_size 0',
        '-f hls'
    ]).output(Helpers.publicPath(`video/${filename[0]}/index.m3u8`)).on('end', async () => { 
        console.log('Conversion: ' + filename[0] + ' - done' );
        await Drive.delete(Helpers.tmpPath(`video/${file_name}`))
    }).run();
}

module.exports = { ConvertsVideo }